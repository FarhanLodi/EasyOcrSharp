...
internal static class ModelDownloadManager
{
    ...
    private static HttpClient CreateHttpClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("EasyOcrSharp/1.0");
        return client;
    }
    ...
    {
        StructureEngineException => false,                 // checksum mismatch — retrying won't help
    ...
    private static string ResolveUrl(ModelAsset asset, ModelDownloadOptions options)
    {
        ...
        // EasyOcrSharp's own variable wins; the legacy one still works so existing air-gapped
        // deployments that mirror these models keep running untouched.
        if (string.IsNullOrWhiteSpace(baseUrl))
            baseUrl = Environment.GetEnvironmentVariable("EASYOCRSHARP_STRUCTURE_MODEL_BASE_URL")
                   ?? Environment.GetEnvironmentVariable("PADDLEOCRNET_MODEL_BASE_URL");
    ...
    {
        var envOverride = Environment.GetEnvironmentVariable("EASYOCRSHARP_STRUCTURE_CACHE")
                       ?? Environment.GetEnvironmentVariable("PADDLEOCRNET_CACHE");
